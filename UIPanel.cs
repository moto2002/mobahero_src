...
public class UIPanel : UIRect
{
	...
	private List<UIEffectSort> container = new List<UIEffectSort>();

	private bool mForced;

	private bool _hasRequested;

	private List<RenderQueueMgr> listRenderQueueManage;
		...
		if (UIPanel.mUpdateFrame != Time.frameCount)
		{
			...
			for (int j = 0; j < UIPanel.list.size; j++)
			{
				...
				else if (uIPanel.renderQueue == UIPanel.RenderQueue.StartAt)
				{
					...
					{
						num = Mathf.Max(num, uIPanel.startingRenderQueue + uIPanel.drawCalls.size * 2);
					}
				}
				else
				{
					...
					if (uIPanel.drawCalls.size != 0)
					{
						num = Mathf.Max(num, uIPanel.startingRenderQueue + 2);
					}
				}
				uIPanel.UpdateUISort();
				num = uIPanel.setUpdateRenderQueueManageValue(num + 1);
			}
		}
	}
			...
			if (uIWidget.isVisible && uIWidget.hasVertices)
			{
				...
				if (material != material2 || texture != mainTexture || shader != shader2)
				{
					if (uIDrawCall != null && uIDrawCall.verts.size != 0)
					{
						...
						uIDrawCall.UpdateGeometry(this);
						uIDrawCall = null;
					}
					material = material2;
					texture = mainTexture;
	...
	private void UpdateDrawCalls()
	{
		...
		for (int i = 0; i < this.drawCalls.size; i++)
		{
			...
			uIDrawCall.renderQueue = ((this.renderQueue != UIPanel.RenderQueue.Explicit) ? (this.startingRenderQueue + i * 2) : this.startingRenderQueue);
	...
	{
		bool hasRequested = this._hasRequested;
	...
	public void addRenderQueueManage(RenderQueueMgr renderQueueManage)
	{
		if (this.listRenderQueueManage == null)
		{
			this.listRenderQueueManage = new List<RenderQueueMgr>();
		}
		if (!this.listRenderQueueManage.Contains(renderQueueManage))
		{
			this.listRenderQueueManage.Add(renderQueueManage);
		}
	}

	public void removeRenderQueueManage(RenderQueueMgr renderQueueManage)
	{
		if (this.listRenderQueueManage == null)
		{
			return;
		...
		if (this.listRenderQueueManage.Contains(renderQueueManage))
		{
			this.listRenderQueueManage.Remove(renderQueueManage);
		}
	}
	...
	private int setUpdateRenderQueueManageValue(int value)
	{
		...
		if (this.listRenderQueueManage != null)
		{
			foreach (RenderQueueMgr current in this.listRenderQueueManage)
			{
				if (current.setRenderQueueValue(num))
				{
					num = value + current.getAddMaxRenderQueueValue();
				}
			}
		}
		return num;
