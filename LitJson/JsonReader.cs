...
namespace LitJson
{
	public class JsonReader
	{
		...
		{
			JsonReader.parse_table = new Hashtable();
		...
		private void ProcessNumber(string number)
		{
			...
			try
			{
				...
				double num = double.Parse(number, numberFormat);
		...
		public bool Read()
		{
			...
			while (!this.parser_return)
			{
				...
				if (this.current_symbol == this.current_input)
				{
					if (!this.ReadToken())
					{
						if ((int)this.automaton_stack.Peek() != 65553)
						{
				...
				else
				{
					...
					try
					{
						array = (int[])(JsonReader.parse_table[this.current_symbol] as IDictionary)[this.current_input];
					}
