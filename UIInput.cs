...
public class UIInput : MonoBehaviour
{
	...
	protected char Validate(string text, int pos, char ch)
	{
		...
		else if (this.validation == UIInput.Validation.Customize)
		{
			if (ch >= 'A' && ch <= 'Z')
			{
				return ch;
		...
		else if (this.validation == UIInput.Validation.CustomizeCustomName)
		{
			char c3 = (text.Length <= 0) ? ' ' : text[Mathf.Clamp(pos, 0, text.Length - 1)];
			char c4 = (text.Length <= 0) ? '\n' : text[Mathf.Clamp(pos + 1, 0, text.Length - 1)];
			if (ch == '\'')
			{
				if (c3 != ' ' && c3 != '\'' && c4 != '\'' && !text.Contains("'"))
				{
					return ch;
				}
			}
		...
		else if (this.validation == UIInput.Validation.Chat && ch != '\0')
		{
			if (ch == '\n')
			{
				return '\0';
