	...
	public class NavmeshCut : MonoBehaviour
	{
			...
			case NavmeshCut.MeshType.CustomMesh:
				if (!(this.mesh == null))
				{
					...
					if (this.useRotation)
					{
						...
						this.bounds = new Bounds(localToWorldMatrix2.MultiplyPoint3x4(this.center + bounds.center), Vector3.zero);
						Vector3 max = bounds.max;
						Vector3 min = bounds.min;
						this.bounds.Encapsulate(localToWorldMatrix2.MultiplyPoint3x4(this.center + new Vector3(max.x, max.y, max.z)));
						this.bounds.Encapsulate(localToWorldMatrix2.MultiplyPoint3x4(this.center + new Vector3(min.x, max.y, max.z)));
						this.bounds.Encapsulate(localToWorldMatrix2.MultiplyPoint3x4(this.center + new Vector3(min.x, max.y, min.z)));
						this.bounds.Encapsulate(localToWorldMatrix2.MultiplyPoint3x4(this.center + new Vector3(max.x, max.y, min.z)));
						this.bounds.Encapsulate(localToWorldMatrix2.MultiplyPoint3x4(this.center + new Vector3(max.x, min.y, max.z)));
						this.bounds.Encapsulate(localToWorldMatrix2.MultiplyPoint3x4(this.center + new Vector3(min.x, min.y, max.z)));
						this.bounds.Encapsulate(localToWorldMatrix2.MultiplyPoint3x4(this.center + new Vector3(min.x, min.y, min.z)));
						this.bounds.Encapsulate(localToWorldMatrix2.MultiplyPoint3x4(this.center + new Vector3(max.x, min.y, min.z)));
		...
		public void GetContour(List<List<IntPoint>> buffer)
		{
			...
			case NavmeshCut.MeshType.Rectangle:
			{
				...
				{
					Matrix4x4 localToWorldMatrix = this.tr.localToWorldMatrix;
					list.Add(this.V3ToIntPoint(localToWorldMatrix.MultiplyPoint3x4(this.center + new Vector3(-this.rectangleSize.x, 0f, -this.rectangleSize.y) * 0.5f)));
					list.Add(this.V3ToIntPoint(localToWorldMatrix.MultiplyPoint3x4(this.center + new Vector3(this.rectangleSize.x, 0f, -this.rectangleSize.y) * 0.5f)));
					list.Add(this.V3ToIntPoint(localToWorldMatrix.MultiplyPoint3x4(this.center + new Vector3(this.rectangleSize.x, 0f, this.rectangleSize.y) * 0.5f)));
					list.Add(this.V3ToIntPoint(localToWorldMatrix.MultiplyPoint3x4(this.center + new Vector3(-this.rectangleSize.x, 0f, this.rectangleSize.y) * 0.5f)));
				}
			...
			case NavmeshCut.MeshType.Circle:
			{
				...
				if (this.useRotation)
				{
					...
					{
						list.Add(this.V3ToIntPoint(localToWorldMatrix2.MultiplyPoint3x4(this.center + new Vector3(Mathf.Cos((float)(i * 2) * 3.14159274f / (float)this.circleResolution), 0f, Mathf.Sin((float)(i * 2) * 3.14159274f / (float)this.circleResolution)) * this.circleRadius)));
					}
				}
				else
				{
					...
					{
						list.Add(this.V3ToIntPoint(a + new Vector3(Mathf.Cos((float)(j * 2) * 3.14159274f / (float)this.circleResolution), 0f, Mathf.Sin((float)(j * 2) * 3.14159274f / (float)this.circleResolution)) * this.circleRadius));
					}
				}
				buffer.Add(list);
				break;
				...
				if (this.contours != null)
				{
					...
					for (int k = 0; k < this.contours.Length; k++)
					{
						...
						if (this.useRotation)
						{
							...
							{
								list.Add(this.V3ToIntPoint(localToWorldMatrix3.MultiplyPoint3x4(this.center + array[l] * this.meshScale)));
							}
						}
						else
						{
							...
							{
								list.Add(this.V3ToIntPoint(a + array[m] * this.meshScale));
							}
						}
						if (flag)
						{
