...
public class UIScrollView : MonoBehaviour
{
	...
	public bool RestritMove;

	public UIScrollView.MoveEvent moveevent;
	...
	public virtual Bounds bounds
	{
		get
		{
			if (!this.mCalculatedBounds)
			{
				...
				this.mBounds = NGUIMath.CalculateRelativeWidgetBounds(this.mTrans, this.mTrans, this.considerInactive);
			}
			return this.mBounds;
		}
	}
	...
	public void MoveAbsolute(Vector3 absolute)
	{
		if (this.RestritMove)
		{
			if ((double)absolute.y > 2.5 || (double)absolute.y < -2.5)
			{
				return;
			}
	...
	public void Drag(Vector3 offset)
	{
		if (base.enabled && NGUITools.GetActive(base.gameObject) && this.mShouldMove)
		{
			...
			if (!this.iOSDragEmulation || this.dragEffect != UIScrollView.DragEffect.MomentumAndSpring)
			{
				this.MoveAbsolute(offset);
			}
			else if (this.mPanel.CalculateConstrainOffset(this.bounds.min, this.bounds.max).magnitude > 1f)
			{
				this.MoveAbsolute(offset * 0.5f);
				this.mMomentum *= 0.5f;
			}
			else
			{
