	...
	internal class NCommand : IComparable<NCommand>
	{
		...
		internal byte[] SerializeHeader()
		{
			...
			else
			{
				...
				this.commandHeader = new byte[this.SizeOfHeader];
				this.commandHeader[0] = this.commandType;
				this.commandHeader[1] = this.commandChannelID;
				this.commandHeader[2] = this.commandFlags;
				this.commandHeader[3] = this.reservedByte;
				int num = 4;
				Protocol.Serialize(this.Size, this.commandHeader, ref num);
				Protocol.Serialize(this.reliableSequenceNumber, this.commandHeader, ref num);
				bool flag4 = this.commandType == 7;
				if (flag4)
				{
					num = 12;
					Protocol.Serialize(this.unreliableSequenceNumber, this.commandHeader, ref num);
				}
				else
				{
					...
					{
						num = 12;
						Protocol.Serialize(this.startSequenceNumber, this.commandHeader, ref num);
						Protocol.Serialize(this.fragmentCount, this.commandHeader, ref num);
						Protocol.Serialize(this.fragmentNumber, this.commandHeader, ref num);
						Protocol.Serialize(this.totalLength, this.commandHeader, ref num);
						Protocol.Serialize(this.fragmentOffset, this.commandHeader, ref num);
					}
				}
				result = this.commandHeader;
			}
