...
namespace ExitGames.Client.Photon
{
	internal class EnetPeer : PeerBase
	{
		...
		internal override int QueuedIncomingCommandsCount
		{
			...
			{
				int num = 0;
				Dictionary<byte, EnetChannel> obj = this.channels;
				lock (obj)
				{
					foreach (EnetChannel current in this.channels.Values)
					{
						num += current.incomingReliableCommandsList.Count;
						num += current.incomingUnreliableCommandsList.Count;
					}
				}
				return num;
			}
		...
		internal override int QueuedOutgoingCommandsCount
		{
			get
			{
				...
				lock (obj)
				{
					foreach (EnetChannel current in this.channels.Values)
					{
						num += current.outgoingReliableCommandsList.Count;
						num += current.outgoingUnreliableCommandsList.Count;
					}
				}
				return num;
			}
		...
		internal override void InitPeerBase()
		{
			...
			Dictionary<byte, EnetChannel> obj2 = this.channels;
			lock (obj2)
			{
				this.channels[255] = new EnetChannel(255, this.commandBufferSize);
				for (byte b = 0; b < base.ChannelCount; b += 1)
				{
					this.channels[b] = new EnetChannel(b, this.commandBufferSize);
				}
				this.channelArray = new EnetChannel[(int)(base.ChannelCount + 1)];
				int num = 0;
				foreach (EnetChannel current in this.channels.Values)
				{
					this.channelArray[num++] = current;
				}
			}
			...
			Queue<NCommand> obj4 = this.outgoingAcknowledgementsList;
			lock (obj4)
			{
		...
		internal override void Disconnect()
		{
			...
			if (!flag)
			{
				bool flag2 = this.outgoingAcknowledgementsList != null;
				if (flag2)
				{
					Queue<NCommand> obj = this.outgoingAcknowledgementsList;
					lock (obj)
					{
						this.outgoingAcknowledgementsList.Clear();
					}
				}
				bool flag3 = this.sentReliableCommands != null;
				if (flag3)
				{
				...
				Dictionary<byte, EnetChannel> obj3 = this.channels;
				lock (obj3)
				{
					foreach (EnetChannel current in this.channels.Values)
					{
		...
		private int GetFragmentLength()
		{
			...
			if (flag)
			{
				...
				num -= 48 - (base.mtu - num);
				num -= 7;
				num -= EnetPeer.HMAC_SIZE + EnetPeer.IV_SIZE;
				num--;
				result = num;
			}
			else
		...
		{
			int num = this.udpBuffer.Length;
			bool flag = this.datagramEncryptedConnection;
			if (flag)
			{
				int num2 = num / EnetPeer.BLOCK_SIZE * EnetPeer.BLOCK_SIZE;
				num = num2 - 1;
				num -= 7 - (this.udpBuffer.Length - num2) + EnetPeer.HMAC_SIZE + EnetPeer.IV_SIZE;
			}
			return num;
		}
			...
			else
			{
				...
				else
				{
					...
					lock (obj)
					{
						...
						Queue<NCommand> obj2 = this.outgoingAcknowledgementsList;
						lock (obj2)
						{
							bool flag3 = this.outgoingAcknowledgementsList.Count > 0;
							if (flag3)
							{
						...
						if (flag4)
						{
							...
							lock (obj3)
							{
								...
								{
									bool flag5 = current != null && current.roundTripTimeout != 0 && this.timeInt - current.commandSentTime > current.roundTripTimeout;
									if (flag5)
									{
						...
						else
						{
							...
							if (trafficStatsEnabled)
							{
								TrafficStats expr_1C9 = base.TrafficStatsOutgoing;
								int totalPacketCount = expr_1C9.TotalPacketCount;
								expr_1C9.TotalPacketCount = totalPacketCount + 1;
			...
			else
			{
				...
				else
				{
					...
					lock (obj)
					{
						...
						if (flag11)
						{
							Dictionary<byte, EnetChannel> obj4 = this.channels;
							lock (obj4)
							{
								for (int i = 0; i < this.channelArray.Length; i++)
								{
						...
						else
						{
							...
							{
								TrafficStats expr_4A2 = base.TrafficStatsOutgoing;
								int totalPacketCount = expr_4A2.TotalPacketCount;
								expr_4A2.TotalPacketCount = totalPacketCount + 1;
								base.TrafficStatsOutgoing.TotalCommandsInPackets += (int)this.udpCommandCount;
							}
							this.SendData(this.udpBuffer, this.udpBufferIndex);
							result = (num > 0);
		...
		private bool AreReliableCommandsInTransit()
		{
			...
			lock (obj)
			{
				foreach (EnetChannel current in this.channels.Values)
				{
					bool flag = current.outgoingReliableCommandsList.Count > 0;
					if (flag)
					{
						result = true;
						return result;
		...
		internal override byte[] SerializeOperationToMessage(byte opc, Dictionary<byte, object> parameters, PeerBase.EgMessageType messageType, bool encrypt)
		{
			...
			lock (serializeMemStream)
			{
				...
				if (flag2)
				{
					byte[] array = this.SerializeMemStream.ToArray();
					array = this.CryptoProvider.Encrypt(array);
					this.SerializeMemStream.Position = 0L;
					this.SerializeMemStream.SetLength(0L);
					this.SerializeMemStream.Write(EnetPeer.messageHeader, 0, EnetPeer.messageHeader.Length);
		...
		internal int SerializeToBuffer(Queue<NCommand> commandList)
		{
			while (commandList.Count > 0)
			{
				...
				else
				{
					...
					Buffer.BlockCopy(nCommand.SerializeHeader(), 0, this.udpBuffer, this.udpBufferIndex, nCommand.SizeOfHeader);
					this.udpBufferIndex += nCommand.SizeOfHeader;
		...
		internal void QueueSentCommand(NCommand command)
		{
			...
			bool flag5 = this.sentReliableCommands.Count >= this.warningSize && this.sentReliableCommands.Count % this.warningSize == 0;
			if (flag5)
			{
		...
		internal void QueueOutgoingReliableCommand(NCommand command)
		{
			...
			EnetChannel obj = enetChannel;
			lock (obj)
			{
				Queue<NCommand> outgoingReliableCommandsList = enetChannel.outgoingReliableCommandsList;
				bool flag = outgoingReliableCommandsList.Count >= this.warningSize && outgoingReliableCommandsList.Count % this.warningSize == 0;
				if (flag)
				{
				...
				if (flag2)
				{
					EnetChannel expr_6E = enetChannel;
					int num = expr_6E.outgoingReliableSequenceNumber + 1;
					expr_6E.outgoingReliableSequenceNumber = num;
		...
		internal void QueueOutgoingUnreliableCommand(NCommand command)
		{
			...
			bool flag = outgoingUnreliableCommandsList.Count >= this.warningSize && outgoingUnreliableCommandsList.Count % this.warningSize == 0;
			if (flag)
			{
			...
			EnetChannel expr_70 = enetChannel;
			int num = expr_70.outgoingUnreliableSequenceNumber + 1;
			expr_70.outgoingUnreliableSequenceNumber = num;
		...
		internal void QueueOutgoingAcknowledgement(NCommand command)
		{
			...
			lock (obj)
			{
				bool flag = this.outgoingAcknowledgementsList.Count >= this.warningSize && this.outgoingAcknowledgementsList.Count % this.warningSize == 0;
				if (flag)
				{
				...
				else
				{
					...
					for (int i = 0; i < (int)b2; i++)
					{
						...
						if (flag13)
						{
							...
							NCommand nCommand = NCommand.CreateAck(this, readCommand, this.serverSentTime);
							this.QueueOutgoingAcknowledgement(nCommand);
							bool trafficStatsEnabled2 = base.TrafficStatsEnabled;
							if (trafficStatsEnabled2)
							{
					...
					else
					{
						...
						if (flag18)
						{
							...
							if (flag19)
							{
								...
								{
									bool flag20 = enetChannel.ContainsReliableSequenceNumber(num++);
									if (flag20)
									{
										command.fragmentsRemaining--;
									}
							...
							{
								bool flag21 = enetChannel.ContainsReliableSequenceNumber(command.startSequenceNumber);
								if (flag21)
								{
			...
			else
			{
				...
				if (flag3)
				{
					base.Listener.DebugReturn(DebugLevel.ALL, string.Concat(new object[]
					{
						...
						"/",
						enetChannel.incomingUnreliableSequenceNumber
					}));
				}
				bool flag4 = command.commandFlags == 1;
				if (flag4)
				{
					bool flag5 = command.reliableSequenceNumber <= enetChannel.incomingReliableSequenceNumber;
					if (flag5)
					{
					...
					{
						bool flag7 = enetChannel.ContainsReliableSequenceNumber(command.reliableSequenceNumber);
						if (flag7)
						{
						...
						{
							bool flag9 = enetChannel.incomingReliableCommandsList.Count >= this.warningSize && enetChannel.incomingReliableCommandsList.Count % this.warningSize == 0;
							if (flag9)
							{
				...
				else
				{
					...
					if (flag10)
					{
						bool flag11 = command.reliableSequenceNumber < enetChannel.incomingReliableSequenceNumber;
						if (flag11)
						{
							bool flag12 = base.debugOut >= DebugLevel.INFO;
							if (flag12)
						...
						{
							bool flag13 = command.unreliableSequenceNumber <= enetChannel.incomingUnreliableSequenceNumber;
							if (flag13)
							{
							...
							{
								bool flag15 = enetChannel.ContainsUnreliableSequenceNumber(command.unreliableSequenceNumber);
								if (flag15)
								{
									...
									if (flag16)
									{
										base.Listener.DebugReturn(DebugLevel.INFO, string.Concat(new object[]
										{
											...
											enetChannel.incomingUnreliableCommandsList[command.unreliableSequenceNumber],
											"/",
											command
										}));
									}
								...
								{
									bool flag17 = enetChannel.incomingUnreliableCommandsList.Count >= this.warningSize && enetChannel.incomingUnreliableCommandsList.Count % this.warningSize == 0;
									if (flag17)
									{
